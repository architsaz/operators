...
# CFLAGS = -pg -Iinclude # the run program to generate the gmon.out and open it with command gprof program gmon.out > report.txt
 ...
 LIB_DIR = lib
# LDFLAGS = -lm
  LDFLAGS = -L$(MYLIB_DIR) -loperator -lmesh -lvector -lCGsolver -lCRSmat -lListDS -lm# -L$(LOC_LIB_DIR) -lListDS  # Linker flags to specify libraries
